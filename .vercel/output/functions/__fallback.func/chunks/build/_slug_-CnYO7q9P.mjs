...
var _slug__vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	async setup(__props) {
		...
		const { data: allServices } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(`jasa-${slug}`, () => queryCollection("services").all())), __temp = await __temp, __restore(), __temp);
		const service = computed(() => (Array.isArray(allServices.value) ? allServices.value : []).find((s) => s.slug === slug));
		useHead$1({
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (unref(service)) {
				_push(`<div${ssrRenderAttrs(mergeProps({ class: "mx-auto max-w-4xl px-5 py-12 md:px-8 md:py-16" }, _attrs))}>`);
				_push(ssrRenderComponent(_component_Breadcrumb, {
					...
					items: [
						...
						{ label: unref(service).title }
					]
				}, null, _parent));
				_push(`<p class="eyebrow mb-4 text-accent"><span class="inline-block h-1 w-1 rounded-full bg-accent"></span> Layanan </p><h1 class="display text-4xl text-ink md:text-5xl">${ssrInterpolate(unref(service).title)}</h1><div class="prose-tech mt-10 max-w-none">`);
				_push(ssrRenderComponent(_component_ContentRenderer, { value: unref(service) }, null, _parent));
				...
				_push(ssrRenderComponent(_component_Button, {
					...
					default: withCtx((_, _push, _parent, _scopeId) => {
						...
						else return [createTextVNode(" Konsultasi Jasa Ini ")];
					}),
					_: 1
				}, _parent));
				_push(`</div></div>`);
			} else {
				_push(`<div${ssrRenderAttrs(mergeProps({ class: "container-tech py-24 md:py-32 text-center" }, _attrs))}><h1 class="display text-3xl text-ink md:text-4xl">Jasa Tidak Ditemukan</h1><div class="mt-8 flex justify-center">`);
				_push(ssrRenderComponent(_component_Button, {
					to: "/jasa",
					variant: "ghost",
					arrow: ""
					...
					default: withCtx((_, _push, _parent, _scopeId) => {
						...
						else return [createTextVNode("Kembali ke Jasa")];
					}),
					_: 1
				}, _parent));
				_push(`</div></div>`);
