...
import { defineComponent, computed, mergeProps, withCtx, createVNode, unref, ref, resolveDirective, useSSRContext } from 'vue';
...
var StatisticsSection_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		const hero = computed(() => [...props.items].sort((a, b) => num(b.value) - num(a.value))[0]);
		...
		const heroDisplay = computed(() => ticked.value === null ? String(heroTarget.value) : String(ticked.value));
		const sparkFrom = computed(() => reduced.value ? { duration: .001 } : {
			strokeDashoffset: 460,
			duration: DUR.draw,
			ease: EASE_OUT_GSAP
		});
		const sparkFillFrom = computed(() => reduced.value ? { duration: .001 } : {
			opacity: 0,
			duration: .6,
			delay: .45,
			ease: EASE_OUT_GSAP
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			let _temp0, _temp1;
			_push(`<section${ssrRenderAttrs(mergeProps({ class: "bg-paper" }, _attrs))}><div class="frame border-b border-line"><div class="border-b border-line px-6 py-14 md:px-10 md:py-16"><h2 class="display max-w-xl text-3xl text-ink md:text-[2.6rem]"> Dipercaya oleh ratusan industri di seluruh Indonesia </h2></div>`);
			if (unref(hero)) _push(`<div class="grid border-b border-line md:grid-cols-2"><div class="flex min-h-[280px] flex-col justify-between px-6 py-8 md:min-h-[340px] md:px-10 md:py-10"><p class="text-sm font-medium text-muted">Total ${ssrInterpolate(unref(hero).label.toLowerCase())}</p><p class="display text-6xl tabular-nums text-ink md:text-8xl">${ssrInterpolate(unref(heroDisplay))}${ssrInterpolate(unref(heroSuffix))}</p></div><div class="flex flex-col border-t border-line px-6 py-8 md:border-l md:border-t-0 md:px-8"><span class="inline-flex w-fit items-center gap-2 rounded-md border border-line bg-white px-3 py-1.5 text-[13px] font-medium text-ink/80 shadow-sm"><span class="h-1.5 w-1.5 rounded-full bg-accent"></span> Pertumbuhan proyek <svg class="h-3 w-3 text-ink/40" viewBox="0 0 12 12" aria-hidden="true"><path d="M3 5l3 3 3-3" fill="none" stroke="currentColor" stroke-width="1.5"></path></svg></span><svg class="mt-6 h-40 w-full flex-1 md:h-auto" viewBox="0 0 400 150" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="stat-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="rgb(228 87 27)" stop-opacity="0.22"></stop><stop offset="1" stop-color="rgb(228 87 27)" stop-opacity="0"></stop></linearGradient></defs><path${ssrRenderAttrs(_temp0 = mergeProps({
				d: "M0 138 L40 132 L80 134 L120 122 L160 126 L200 104 L240 110 L280 78 L320 84 L360 40 L400 14 L400 150 L0 150 Z",
				fill: "url(#stat-fill)"
			}, ssrGetDirectiveProps(_ctx, _directive_gsap, unref(sparkFillFrom), void 0, {
				whenVisible: true,
				once: true,
				from: true
			})))}>${"textContent" in _temp0 ? ssrInterpolate(_temp0.textContent) : _temp0.innerHTML ?? ""}</path><path${ssrRenderAttrs(_temp1 = mergeProps({
			...
			}, ssrGetDirectiveProps(_ctx, _directive_gsap, unref(sparkFrom), void 0, {
				whenVisible: true,
				once: true,
				from: true
			...
			ssrRenderList(unref(rest), (item, i) => {
...
var WhyChooseUsSection_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<section${ssrRenderAttrs(mergeProps({ class: "bg-paper" }, _attrs))}><div class="frame border-b border-line"><div class="border-b border-line px-6 py-14 md:px-10 md:py-16">`);
			...
			_push(ssrRenderComponent(_component_NuxtLink, {
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					...
					else return [createVNode("span", { class: "text-lg leading-none text-accent" }, "→"), createVNode("div", null, [createVNode("h3", { class: "text-lg font-semibold text-ink" }, "Butuh bantuan memilih?"), createVNode("p", { class: "mt-2 text-sm text-muted transition-colors group-hover:text-accent" }, " Konsultasi gratis dengan tim kami ")])];
				}),
				_: 1
			}, _parent));
			_push(`<!--[-->`);
			ssrRenderList(unref(fillerCount), (n) => {
