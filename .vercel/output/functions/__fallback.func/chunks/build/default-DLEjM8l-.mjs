...
import { defineComponent, withAsyncContext, computed, unref, ref, mergeProps, withCtx, createVNode, watch, createTextVNode, toDisplayString, openBlock, createBlock, createCommentVNode, resolveDynamicComponent, useSSRContext } from 'vue';
...
var AnnouncementBar_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (unref(visible)) {
				_push(`<div${ssrRenderAttrs(mergeProps({ class: "relative isolate border-b border-line bg-paper-soft text-ink" }, _attrs))}><div class="flex items-center gap-3 px-4 py-2 md:px-6">`);
				_push(ssrRenderComponent(_component_NuxtLink, {
					...
					default: withCtx((_, _push, _parent, _scopeId) => {
						if (_push) _push(`<span class="h-1.5 w-1.5 shrink-0 rounded-full bg-accent" aria-hidden="true"${_scopeId}></span><span class="truncate"${_scopeId}>Konsultasi &amp; survei lokasi gratis — hubungi tim kami</span><span class="grid h-4 w-5 shrink-0 place-items-center rounded-sm bg-accent/15 text-[10px] text-accent transition-transform group-hover:translate-x-0.5"${_scopeId}>→</span>`);
							...
							createVNode("span", { class: "truncate" }, "Konsultasi & survei lokasi gratis — hubungi tim kami"),
							createVNode("span", { class: "grid h-4 w-5 shrink-0 place-items-center rounded-sm bg-accent/15 text-[10px] text-accent transition-transform group-hover:translate-x-0.5" }, "→")
						];
					}),
					_: 1
				}, _parent));
...
var BrandLogo_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	__name: "BrandLogo",
	__ssrInlineRender: true,
	props: {
		to: { default: "/" },
			...
			_push(ssrRenderComponent(NuxtLink, mergeProps({
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`<span class="text-[17px]"${_scopeId}>BBS</span><span class="text-[17px]"${_scopeId}>(</span><svg viewBox="0 0 24 24" class="h-[13px] w-[13px] self-center" fill="none" stroke="currentColor" stroke-width="2.2" aria-hidden="true"${_scopeId}><circle cx="12" cy="12" r="7.5"${_scopeId}></circle><circle cx="12" cy="12" r="2.5" fill="currentColor" stroke="none"${_scopeId}></circle><path d="M12 1.8v3M12 19.2v3M1.8 12h3M19.2 12h3M4.8 4.8l2.1 2.1M17.1 17.1l2.1 2.1M19.2 4.8l-2.1 2.1M6.9 17.1l-2.1 2.1" stroke-linecap="round"${_scopeId}></path></svg><span class="text-[17px]"${_scopeId}>)</span>`);
						...
						createVNode("span", { class: "text-[17px]" }, "BBS"),
						createVNode("span", { class: "text-[17px]" }, "("),
...
var NavigationDropdown_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		const isActive = computed(() => props.items.some((item) => route.path.startsWith(item.href)));
		watch(() => route.path, () => {
			isOpen.value = false;
			activeIndex.value = -1;
		});
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(ssrRenderComponent(_component_NuxtLink, {
				to: __props.href,
				class: ["relative flex items-center gap-1.5 px-3 py-2 text-sm font-medium transition-colors", unref(isActive) ? "text-ink" : "text-ink/60 hover:text-ink"]
			}, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`${ssrInterpolate(__props.label)} <svg class="${ssrRenderClass([{ "rotate-180": unref(isOpen) }, "h-3 w-3 transition-transform duration-200"])}" viewBox="0 0 12 12" aria-hidden="true"${_scopeId}><path d="M3 5l3 3 3-3" fill="none" stroke="currentColor" stroke-width="1.5"${_scopeId}></path></svg>`);
						if (unref(isActive)) _push(`<span class="absolute -bottom-[3px] left-3 right-3 h-px bg-accent"${_scopeId}></span>`);
						else _push(`<!---->`);
					} else return [
						createTextVNode(toDisplayString(__props.label) + " ", 1),
						(openBlock(), createBlock("svg", {
							class: ["h-3 w-3 transition-transform duration-200", { "rotate-180": unref(isOpen) }],
							viewBox: "0 0 12 12",
							"aria-hidden": "true"
						}, [createVNode("path", {
							d: "M3 5l3 3 3-3",
						...
						unref(isActive) ? (openBlock(), createBlock("span", {
							key: 0,
			...
			if (unref(isOpen)) {
				...
				ssrRenderList(__props.items, (item, i) => {
					_push(ssrRenderComponent(_component_NuxtLink, {
						key: item.label,
						to: item.href,
						class: ["flex items-center gap-2.5 rounded-lg px-3 py-2 text-[13.5px] font-medium transition-colors", i === unref(activeIndex) || unref(route).path.startsWith(item.href) ? "bg-paper-soft text-ink" : "text-ink/60 hover:bg-paper-soft hover:text-ink"]
					}, {
						default: withCtx((_, _push, _parent, _scopeId) => {
							...
							else return [createVNode("span", { class: "text-[10px] font-semibold tabular-nums text-accent/70" }, toDisplayString(String(i + 1).padStart(2, "0")), 1), createTextVNode(" " + toDisplayString(item.label), 1)];
						}),
						_: 2
					}, _parent));
				});
			...
			_push(ssrRenderComponent(_component_NuxtLink, {
				to: __props.href,
				class: ["py-2 text-[15px] font-medium transition-colors", unref(isActive) ? "text-ink" : "text-ink/60 hover:text-ink"]
			}, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`${ssrInterpolate(__props.label)}`);
					else return [createTextVNode(toDisplayString(__props.label), 1)];
				}),
				_: 1
			}, _parent));
			_push(`<button class="p-2 text-ink/40 transition-colors hover:text-ink" aria-label="Toggle submenu"><svg class="${ssrRenderClass([{ "rotate-180": unref(isOpen) }, "h-3.5 w-3.5 transition-transform duration-200"])}" viewBox="0 0 12 12" aria-hidden="true"><path d="M3 5l3 3 3-3" fill="none" stroke="currentColor" stroke-width="1.5"></path></svg></button></div><div class="mb-2 ml-1 flex flex-col border-l border-line pl-4" style="${ssrRenderStyle(unref(isOpen) ? null : { display: "none" })}"><!--[-->`);
...
var AppNavbar_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<nav${ssrRenderAttrs(mergeProps({ class: "sticky top-0 z-50 border-b border-line bg-white/85 backdrop-blur-xl" }, _attrs))}><div class="frame flex h-16 items-center gap-6 px-5 md:px-8">`);
			...
			ssrRenderList(__props.navItems, (item) => {
				...
				if (item.children) _push(ssrRenderComponent(_component_NavigationDropdown, {
					...
					default: withCtx((_, _push, _parent, _scopeId) => {
						...
						} else return [createTextVNode(toDisplayString(item.label) + " ", 1), isActive(item) ? (openBlock(), createBlock("span", {
							key: 0,
							class: "absolute -bottom-[3px] left-3 right-3 h-px bg-accent"
						})) : createCommentVNode("", true)];
					}),
			...
			_push(ssrRenderComponent(_component_Button, {
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					...
					else return [createTextVNode(toDisplayString(__props.ctaLabel), 1)];
				}),
				_: 1
			}, _parent));
			_push(`</div><button class="ml-auto flex flex-col gap-1.5 p-2 lg:hidden" aria-label="Buka menu"><span class="${ssrRenderClass([unref(isOpen) ? "translate-y-2 rotate-45" : "", "block h-0.5 w-5 bg-ink transition-transform duration-200"])}"></span><span class="${ssrRenderClass([unref(isOpen) ? "opacity-0" : "", "block h-0.5 w-5 bg-ink transition-opacity duration-200"])}"></span><span class="${ssrRenderClass([unref(isOpen) ? "-translate-y-2 -rotate-45" : "", "block h-0.5 w-5 bg-ink transition-transform duration-200"])}"></span></button></div><div class="border-t border-line bg-white px-5 pb-6 pt-2 lg:hidden" style="${ssrRenderStyle(unref(isOpen) ? null : { display: "none" })}"><!--[-->`);
			ssrRenderList(__props.navItems, (item) => {
				_push(`<!--[-->`);
				if (item.children) _push(ssrRenderComponent(_component_NavigationDropdown, {
					label: item.label,
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			ssrRenderList(__props.footerColumns, (col) => {
				...
				ssrRenderList(col.links, (link) => {
					_push(`<li>`);
					if (!link.href.startsWith("http")) _push(ssrRenderComponent(unref(NuxtLink$1), {
						to: link.href,
			...
			ssrRenderList(socials, (s) => {
				_push(`<li>`);
				ssrRenderVNode(_push, createVNode(resolveDynamicComponent(s.href.startsWith("/") ? unref(NuxtLink$1) : "a"), {
					...
					default: withCtx((_, _push, _parent, _scopeId) => {
						if (_push) {
						...
						} else return [s.icon === "wa" ? (openBlock(), createBlock("svg", {
							key: 0,
							class: "h-4 w-4",
							viewBox: "0 0 24 24",
							fill: "currentColor",
							"aria-hidden": "true"
						}, [createVNode("path", { d: "M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51l-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.71.306 1.263.489 1.694.625.712.227 1.36.195 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" })])) : s.icon === "mail" ? (openBlock(), createBlock("svg", {
							key: 1,
							class: "h-4 w-4",
							viewBox: "0 0 24 24",
							fill: "none",
							...
							height: "14",
							rx: "2.5"
						}), createVNode("path", { d: "M3.5 7l8.5 6 8.5-6" })])) : (openBlock(), createBlock("svg", {
							key: 2,
							class: "h-4 w-4",
							viewBox: "0 0 24 24",
							fill: "none",
							...
							"stroke-width": "1.7",
							"aria-hidden": "true"
						}, [createVNode("path", { d: "M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" }), createVNode("circle", {
							cx: "12",
							cy: "10",
							r: "3"
			...
			_push(`<!--]--></ul></div></div><div class="flex flex-col gap-2 border-t border-white/10 px-6 py-6 text-xs text-white/35 sm:flex-row sm:items-center sm:justify-between md:px-10"><span>© ${ssrInterpolate(unref(year))} CV Bintang Berjaya Satu. All Rights Reserved.</span><span class="font-semibold uppercase tracking-[0.14em]">BBS · Jakarta Timur, Indonesia</span></div></div></footer>`);
		};
	}
});
//#endregion
...
var default_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	async setup(__props) {
		...
		const { data: navData } = ([__temp, __restore] = withAsyncContext(() => useAsyncData("site-navigation", () => queryCollection("navigation").first())), __temp = await __temp, __restore(), __temp);
		const headerNav = computed(() => navData.value?.header ?? []);
		const cta = computed(() => navData.value?.cta ?? {
			label: "Hubungi Kami",
			href: "#"
		});
		const footerCols = computed(() => navData.value?.footer ?? []);
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(ssrRenderComponent(_component_AppNavbar, {
				"nav-items": unref(headerNav),
				"cta-label": unref(cta).label,
				"cta-href": unref(cta).href
			}, null, _parent));
			ssrRenderSlot(_ctx.$slots, "default", {}, null, _push, _parent);
			_push(ssrRenderComponent(_component_AppFooter, { "footer-columns": unref(footerCols) }, null, _parent));
			_push(`<!--]-->`);
		};
	}
});
