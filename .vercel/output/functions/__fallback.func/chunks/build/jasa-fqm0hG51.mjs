...
var ServiceCard_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ vueExports.defineComponent({
	__name: "ServiceCard",
				...
				default: vueExports.withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						vueExports.createVNode("p", { class: "flex items-center gap-2 text-[10.5px] font-semibold uppercase tracking-[0.14em] text-muted" }, [vueExports.createVNode("span", {
							...
							"aria-hidden": "true"
						}), vueExports.createTextVNode(" Jasa ")]),
...
var index_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ vueExports.defineComponent({
	...
	async setup(__props) {
		...
		const { data: services } = ([__temp, __restore] = vueExports.withAsyncContext(() => useAsyncData("jasa-listing", () => queryCollection("services").all())), __temp = await __temp, __restore(), __temp);
		const serviceList = vueExports.computed(() => (Array.isArray(services.value) ? services.value : []).map((s) => ({
			slug: s.slug,
			title: s.title,
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (vueExports.unref(serviceList).length) {
				_push(`<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3"><!--[-->`);
				ssrRenderList(vueExports.unref(serviceList), (s) => {
					_push(ssrRenderComponent(_component_ServiceCard, vueExports.mergeProps({ key: s.slug }, { ref_for: true }, s), null, _parent));
				});
				_push(`<!--]--></div>`);
