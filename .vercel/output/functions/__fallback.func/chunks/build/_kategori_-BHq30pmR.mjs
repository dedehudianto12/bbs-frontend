...
var _kategori__vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	async setup(__props) {
		...
		const { data: allProducts } = ([__temp, __restore] = withAsyncContext(() => useAsyncData(`produk-lainnya-${subcat}`, () => queryCollection("products").all())), __temp = await __temp, __restore(), __temp);
		const otherProducts = computed(() => (Array.isArray(allProducts.value) ? allProducts.value : []).filter((p) => p.group !== "belt-conveyor").map((p) => ({
			slug: p.slug,
			title: p.title,
			category: p.category,
		...
		const products = computed(() => otherProducts.value.filter((p) => p._categoryClean === subcat));
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (unref(isValid)) {
				...
				_push(ssrRenderComponent(_component_PageHero, {
					...
					description: `Produk Lainnya kategori ${unref(currentLabel)}.`
				}, {
				...
				_push(ssrRenderComponent(_component_Button, {
					...
					default: withCtx((_, _push, _parent, _scopeId) => {
						...
						else return [createTextVNode("Kembali ke Produk Lainnya")];
					}),
					_: 1
				}, _parent));
				_push(`</div></div>`);
