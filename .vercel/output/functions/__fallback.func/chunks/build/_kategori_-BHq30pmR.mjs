...
//#region pages/produk/lainnya/[kategori].vue?vue&type=script&setup=true&lang.ts
var _kategori__vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ vueExports.defineComponent({
	...
	async setup(__props) {
		...
		const { data: allProducts } = ([__temp, __restore] = vueExports.withAsyncContext(() => useAsyncData(`produk-lainnya-${subcat}`, () => queryCollection("products").all())), __temp = await __temp, __restore(), __temp);
		const otherProducts = vueExports.computed(() => (Array.isArray(allProducts.value) ? allProducts.value : []).filter((p) => p.group !== "belt-conveyor").map((p) => ({
			slug: p.slug,
			title: p.title,
			category: p.category,
		...
		const products = vueExports.computed(() => otherProducts.value.filter((p) => p._categoryClean === subcat));
		const isValid = vueExports.computed(() => products.value.length > 0);
		const subCategories = vueExports.computed(() => {
			return [...new Set(otherProducts.value.map((p) => p.category))].map((c) => ({
				label: c,
		...
		useHead$1({
			...
			meta: [{
				name: "description",
				content: `${currentLabel.value} berkualitas untuk kebutuhan industri. Tersedia dari BBS Conveyor.`
			}]
		});
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (vueExports.unref(isValid)) {
				...
				_push(ssrRenderComponent(_component_PageHero, {
					...
					description: `Produk Lainnya kategori ${vueExports.unref(currentLabel)}.`
				}, {
				...
				_push(ssrRenderComponent(_component_Button, {
					...
					default: vueExports.withCtx((_, _push, _parent, _scopeId) => {
						if (_push) _push(`Kembali ke Produk Lainnya`);
						else return [vueExports.createTextVNode("Kembali ke Produk Lainnya")];
					}),
					_: 1
				}, _parent));
				_push(`</div></div>`);
...
//#endregion
//#region pages/produk/lainnya/[kategori].vue
...
_kategori__vue_vue_type_script_setup_true_lang_default.setup = (props, ctx) => {
	...
	(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/produk/lainnya/[kategori].vue");
...
//# sourceMappingURL=_kategori_-BHq30pmR.mjs.map
