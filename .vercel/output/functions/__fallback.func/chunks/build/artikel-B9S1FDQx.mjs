...
var index_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	...
	async setup(__props) {
		...
		const tag = ref("");
		const { data: allArticles } = ([__temp, __restore] = withAsyncContext(() => useAsyncData("artikel-listing", () => queryCollection("blog").all())), __temp = await __temp, __restore(), __temp);
		const articleList = computed(() => (Array.isArray(allArticles.value) ? [...allArticles.value].sort((a, b) => b.publishedAt.localeCompare(a.publishedAt)) : []).map((a) => ({
			slug: a.slug,
			title: a.title,
			excerpt: a.excerpt,
			tag: a.tags?.[0] ?? "",
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (unref(featuredArticle) && !unref(tag)) {
				...
				_push(ssrRenderComponent(_component_ArticleCard, unref(featuredArticle), null, _parent));
				_push(`</div>`);
			} else _push(`<!---->`);
			if (unref(allTags).length) {
				_push(`<div class="mb-10 flex flex-wrap gap-2"><button class="${ssrRenderClass([unref(tag) === "" ? "border-accent bg-accent text-white" : "border-line bg-white text-muted hover:border-ink/30 hover:text-ink", "rounded-md border px-3.5 py-1.5 text-xs font-semibold uppercase tracking-[0.1em] transition-colors"])}"> Semua </button><!--[-->`);
				ssrRenderList(unref(allTags), (t) => {
					_push(`<button class="${ssrRenderClass([unref(tag) === t ? "border-accent bg-accent text-white" : "border-line bg-white text-muted hover:border-ink/30 hover:text-ink", "rounded-md border px-3.5 py-1.5 text-xs font-semibold uppercase tracking-[0.1em] transition-colors"])}">${ssrInterpolate(t)}</button>`);
				});
				_push(`<!--]--></div>`);
			} else _push(`<!---->`);
			_push(ssrRenderComponent(_component_ArticleGrid, { articles: unref(filteredArticles) }, null, _parent));
			_push(`</div>`);
