...
var _slug__vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ vueExports.defineComponent({
	...
	async setup(__props) {
		...
		const { data: allProducts } = ([__temp, __restore] = vueExports.withAsyncContext(() => useAsyncData(`product-${slug}`, () => queryCollection("products").all())), __temp = await __temp, __restore(), __temp);
		const product = vueExports.computed(() => (Array.isArray(allProducts.value) ? allProducts.value : []).find((p) => p.slug === slug));
		const categoryLink = vueExports.computed(() => {
			if (!product.value) return "/produk/belt-conveyor";
			return product.value.group === "belt-conveyor" ? "/produk/belt-conveyor" : "/produk/lainnya";
		});
		const waMessage = vueExports.computed(() => {
			if (!product.value) return "";
			return `Halo, saya ingin menanyakan ketersediaan ${product.value.title}`;
		});
		const heroTheme = vueExports.computed(() => {
			let h = 0;
			for (const ch of slug) h = h * 31 + ch.charCodeAt(0) >>> 0;
			return h;
		});
		const relatedProducts = vueExports.computed(() => {
			if (!product.value) return [];
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (vueExports.unref(product)) {
				_push(`<article${ssrRenderAttrs(vueExports.mergeProps({ class: "container-tech py-12 md:py-16" }, _attrs))}>`);
				_push(ssrRenderComponent(_component_Breadcrumb, {
					items: vueExports.unref(breadcrumbItems),
					class: "mb-10"
				}, null, _parent));
				_push(`<div class="grid grid-cols-1 gap-10 md:grid-cols-2 md:gap-14">`);
				_push(ssrRenderComponent(_component_GradientPanel, {
					index: vueExports.unref(heroTheme),
					class: "aspect-[4/3] w-full"
				}, {
					default: vueExports.withCtx((_, _push, _parent, _scopeId) => {
						if (_push) _push(`<div class="absolute inset-0 flex flex-col justify-between p-7"${_scopeId}><span class="text-xs font-semibold uppercase tracking-[0.16em] text-ink/55"${_scopeId}>BBS Conveyor</span><span class="font-display text-3xl leading-tight text-ink"${_scopeId}>${ssrInterpolate(vueExports.unref(product).title)}</span></div>`);
						else return [vueExports.createVNode("div", { class: "absolute inset-0 flex flex-col justify-between p-7" }, [vueExports.createVNode("span", { class: "text-xs font-semibold uppercase tracking-[0.16em] text-ink/55" }, "BBS Conveyor"), vueExports.createVNode("span", { class: "font-display text-3xl leading-tight text-ink" }, vueExports.toDisplayString(vueExports.unref(product).title), 1)])];
					}),
					_: 1
				}, _parent));
				_push(`<div class="flex flex-col justify-center"><span class="inline-block w-fit rounded-full bg-accent/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.1em] text-accent">${ssrInterpolate(vueExports.unref(product).category)}</span><h1 class="display mt-4 text-3xl text-ink md:text-4xl">${ssrInterpolate(vueExports.unref(product).title)}</h1><p class="mt-5 leading-relaxed text-muted">${ssrInterpolate(vueExports.unref(product).excerpt)}</p><div class="mt-8 flex flex-wrap gap-3">`);
				_push(ssrRenderComponent(_component_Button, {
					href: `https://wa.me/${waSales1}?text=${encodeURIComponent(vueExports.unref(waMessage))}`,
					variant: "accent",
					external: ""
				}, {
					default: vueExports.withCtx((_, _push, _parent, _scopeId) => {
				...
				_push(ssrRenderComponent(_component_ContentRenderer, { value: vueExports.unref(product) }, null, _parent));
				_push(`</div></div>`);
				if (vueExports.unref(relatedProducts).length) {
					...
					_push(ssrRenderComponent(_component_ProductGrid, { products: vueExports.unref(relatedProducts) }, null, _parent));
					_push(`</div></div>`);
				} else _push(`<!---->`);
				_push(`</article>`);
			} else {
				_push(`<div${ssrRenderAttrs(vueExports.mergeProps({ class: "container-tech py-24 md:py-32 text-center" }, _attrs))}><h1 class="display text-3xl text-ink md:text-4xl">Produk Tidak Ditemukan</h1><p class="mt-4 text-muted">Produk yang Anda cari tidak tersedia.</p><div class="mt-8 flex justify-center">`);
				_push(ssrRenderComponent(_component_Button, {
					...
					default: vueExports.withCtx((_, _push, _parent, _scopeId) => {
						...
						else return [vueExports.createTextVNode("Kembali ke Produk")];
					}),
					_: 1
				}, _parent));
				_push(`</div></div>`);
