...
var tentang_kami_vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	__name: "tentang-kami",
	__ssrInlineRender: true,
	async setup(__props) {
		let __temp, __restore;
		const { data: whyChooseUs } = ([__temp, __restore] = withAsyncContext(() => useAsyncData("about-why-choose-us", () => queryCollection("homepage").where("section", "=", "why-choose-us").first())), __temp = await __temp, __restore(), __temp);
		const whyChooseUsItems = computed(() => whyChooseUs.value?.items ?? []);
		const { data: hp } = ([__temp, __restore] = withAsyncContext(() => useAsyncData("about-hp", () => queryCollection("homepageConfig").first())), __temp = await __temp, __restore(), __temp);
