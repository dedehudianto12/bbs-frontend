  ...
  <nav class="sticky top-0 z-50 border-b border-line bg-white/85 backdrop-blur-xl">
    <div class="frame flex h-16 items-center gap-6 px-5 md:px-8">
      ...
      <div class="ml-auto hidden items-center gap-3 lg:flex">
        <a
          ...
          class="nav-icon grid h-8 w-8 place-items-center rounded-none text-ink/50 hover:bg-ink/5 hover:text-ink"
          aria-label="WhatsApp"
        >
        ...
        <UiButton :href="ctaHref" size="sm">{{ ctaLabel }}</UiButton>
      </div>
    ...
    >
      <div v-show="isOpen" class="border-t border-line bg-white px-5 pb-6 pt-2 lg:hidden">
        ...
        <div class="mt-5">
          <UiButton :href="ctaHref" block @click="isOpen = false">
            {{ ctaLabel }}
          </UiButton>
        </div>
      </div>
...
<style scoped>
/* Explicit properties only. The icon button is pressable, so it gets the same
   physical acknowledgement as UiButton. */
.nav-icon {
  transition:
