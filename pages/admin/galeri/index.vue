...
const { data: res, refresh, status, error: fetchErr } = await useAsyncData('admin-galeri', () => get<any>(`/admin/galeri?${params.value}`), { server: false })
...
function openCreate(){modalTitle.value='Galeri Baru';editId.value=null;Object.assign(form,{caption:'',location:''});reset();fieldErrors.value={};error.value='';modalOpen.value=true}
async function openEdit(id:string){const r=await get<any>(`/admin/galeri/${id}`);if(r?.data){const g=r.data;editId.value=id;modalTitle.value='Edit Galeri';Object.assign(form,{caption:g.caption,location:g.location??''});reset(g.image??'');fieldErrors.value={};error.value='';modalOpen.value=true}}
async function save(){
...
<template>
  <div>
    ...
    <div class="overflow-x-auto rounded-lg border border-line bg-white"><table class="table-admin"><thead><tr><th>Gambar</th><th>Caption</th><th>Lokasi</th><th>Aksi</th></tr></thead>
    <tbody><tr v-for="g in items" :key="g.id"><td><img v-if="g.image" :src="g.image" :alt="`Thumbnail: ${g.caption}`" width="70" height="44" loading="lazy" decoding="async" class="h-11 w-[70px] rounded object-cover" /><span v-else class="text-muted">—</span></td><td class="font-semibold text-ink">{{ g.caption }}</td><td class="text-muted">{{ g.location??'—' }}</td><td><button @click="openEdit(g.id)" title="Edit" aria-label="Edit" class="mr-2 cursor-pointer rounded border-none bg-transparent p-1 text-muted"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button><button title="Hapus" aria-label="Hapus" class="cursor-pointer rounded border-none bg-transparent p-1 text-muted" @click="handleDelete(g.id,g.caption)"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg></button></td></tr>
    <AdminTableState :colspan="4" :loading="loading" :failed="failed" :empty="!items.length" empty-text="Belum ada galeri." @retry="refresh()" /></tbody></table></div>
