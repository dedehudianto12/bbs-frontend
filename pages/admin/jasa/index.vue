...
const { data: res, refresh } = await useAsyncData('admin-jasa', () => get<any>(`/admin/jasa?${params.value}`))
...
<template>
  <div>
    ...
    <tbody><tr v-for="s in items" :key="s.id" style="border-bottom:1px solid rgb(var(--line))"><td style="padding:14px 16px;font-weight:600;color:rgb(var(--ink))">{{ s.name }}</td><td style="padding:14px 16px;color:rgb(var(--muted));max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ s.shortDescription }}</td><td style="padding:14px 16px"><button @click="openEdit(s.id)" title="Edit" style="color:rgb(var(--muted));margin-right:8px;background:none;border:none;cursor:pointer;padding:4px;border-radius:4px"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button><button title="Hapus" style="color:rgb(var(--muted));background:none;border:none;cursor:pointer;padding:4px;border-radius:4px" @click="handleDelete(s.id,s.name)"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg></button></td></tr>
    <tr v-if="!items.length"><td colspan="3" style="padding:60px 16px;text-align:center;color:rgb(var(--muted))">Belum ada jasa.</td></tr></tbody></table></div>
    ...
    <ModalForm :open="modalOpen" :title="modalTitle" @close="modalOpen=false">
      ...
      <form style="display:flex;flex-direction:column;gap:18px" @submit.prevent="save">
        ...
        <label style="display:block"><span :style="L">Deskripsi Lengkap</span><TiptapEditor v-model="form.fullDescription" /></label>
