...
const { data: productRes, error: productErr } = await useAsyncData(`produk-belt-conveyor-${subcat}`, () =>
...
if (import.meta.client) {
  onMounted(() => {
    if (!productRes.value?.data?.length) refreshNuxtData(`produk-belt-conveyor-${subcat}`)
  })
}
