...
function getKategoriInfo(group: string, kategori: string) {
  ...
  const base = group === 'belt-conveyor' ? '/produk/belt-conveyor' : '/produk/lainnya'
  const match = list.find((k) => k.slug === kategori)
  return { label: match?.label ?? kategori, link: `${base}/${kategori}` }
}
  ...
  <article v-if="product" class="max-w-6xl mx-auto px-4 py-12">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
      ...
      <div>
        ...
        <div class="grid grid-cols-4 gap-2">
          <button
            ...
            :key="i"
            class="aspect-[4/3] bg-bg-soft rounded flex items-center justify-center text-neutral text-xs border-2 transition-colors"
            :class="selectedImage === i ? 'border-gold' : 'border-transparent hover:border-border'"
      ...
      <div>
        ...
        <h1 class="text-2xl md:text-3xl font-bold text-ink mt-1 mb-4">{{ product.name }}</h1>

        <p class="text-neutral leading-relaxed mb-6">{{ product.description }}</p>

        <a
          :href="`https://wa.me/${settings.waSales1}?text=${encodeURIComponent(waMessage)}`"
          target="_blank"
        ...
        <!-- Spesifikasi table -->
        <div class="mt-8">
          <h2 class="text-lg font-semibold text-ink mb-3">Spesifikasi</h2>
          <dl class="border border-border rounded-lg divide-y divide-border">
            <div v-for="(value, key) in product.specs" :key="key" class="flex justify-between px-3 py-2 gap-3">
              <dt class="text-sm text-neutral">{{ key }}</dt>
              <dd class="text-sm text-ink font-medium text-right">{{ value }}</dd>
            </div>
          </dl>
        </div>
      </div>
    ...
    <div class="mt-16">
      ...
      <div class="text-neutral leading-relaxed max-w-3xl">
        <p>{{ product.detail }}</p>
      </div>
    </div>
  </article>
